<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Raleway&display=swap" rel="stylesheet"> 

    <link rel="stylesheet" href="css/base.css">    
    <link rel="stylesheet" href="css/index.css">    
    <link rel="stylesheet" href="css/responsive.css">

    <title>Cards for Humanity - A Cards Against Humanity clone</title>
</head>
<body>
    <main>
        <h1>Cards For Humanity</h1>
        <h2>Cards Against Humanity, for Humanity</h2>
        <p>A Cards Against Humanity clone.</p>

        <div class="card black"><b>Cards Against Humanity</b><p>A party game for ________ people.</p></div>
        <div class="card white">Horrible.</div>

        <h3>What is Cards Against Humanity?</h3>
        <p>Cards Against Humanity is a party game for horrible people. Unlike most of the party games you've played before, Cards Against Humanity is as despicable and awkward as you and your friends.</p>
        <p>The game is simple. Each round, one player asks a question from a black card, and everyone else answers with their funniest white card.</p>

        <form method="post">
            <input type="submit" value="Start a new Game">
        </form>
    </main>
</body>
</html>